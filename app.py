...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS doctors_reg (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
...
symptom_synonyms = {
    ...
    "acidity": "acidity", "heartburn": "acidity", "gas": "excessive_hunger",
    "bloating": "abdominal_pain", "itching": "itching", "itchy": "itching",
    "skin rash": "skin_rash", "rash": "skin_rash", "red skin": "skin_rash",
    "yellow skin": "yellowing_of_skin", "yellowish skin": "yellowing_of_skin",
    ...
    "cant breathe": "breathlessness", "wheezing": "breathlessness",
    ...
    "weak": "weakness", "no energy": "fatigue", "lethargy": "lethargy",
    ...
    "weight loss": "weight_loss", "increased appetite": "excessive_hunger",
    ...
    "nausea": "nausea", "fatigue": "fatigue", "constipation": "constipation",
    "painful walking": "painful_walking", "stiff joints": "stiff_neck",
}
...
def extract_symptoms(user_input, all_symptoms):
    ...
    for phrase, mapped in symptom_synonyms.items():
        if phrase in text:
            if mapped in list(all_symptoms):
...
if page == "🏠 Home & Diagnosis":
    ...
    with st.expander("💡 Tips for better prediction"):
        ...
        st.write("✅ Common words work: tired, itchy, dizzy, swelling, chills")

    symptoms_input = st.text_area(
        "Type your symptoms below",
        placeholder="e.g. I have fever and headache and nausea and joint pain and fatigue",
        height=100)
    ...
    if st.button("🔍 Predict Disease"):
        if not name:
            ...
            if not symptoms_list:
                st.error("Could not detect symptoms. Please try: fever, headache, nausea, fatigue, joint pain")
            else:
                ...
                st.markdown(
                    "<div class='top1-card'>"
                    "<h3>🥇 #1 Most Likely: " + top3[0]['disease'] + "</h3>"
                ...
                if top3[1]['confidence'] > 0:
                    st.markdown(
                        "<div class='top2-card'>"
                        "<h3>🥈 #2 Possible: " + top3[1]['disease'] + "</h3>"
                ...
                if top3[2]['confidence'] > 0:
                    st.markdown(
                        "<div class='top3-card'>"
                        "<h3>🥉 #3 Also Possible: " + top3[2]['disease'] + "</h3>"
                ...
                st.info("The #1 prediction is the most likely disease based on your symptoms.")

                st.markdown("<div class='section-title'>🩺 Main Diagnosis Details</div>", unsafe_allow_html=True)
                desc = description_list.get(top_disease, "No description available.")
                st.markdown(
                    "<div class='result-card'>"
                    "<h4>About " + top_disease + "</h4>"
                    "<p>" + desc + "</p>"
                    "</div>", unsafe_allow_html=True)
                ...
                pdf_path = generate_pdf(name, age, gender, symptoms_list, top_disease, top_confidence, desc, precs_for_pdf)
                ...
                st.markdown(
                    ...
                    "🤖 <b>HealthBot:</b> Based on your symptoms, you most likely have <b>" + top_disease + "</b>. "
    ...
    if st.button("Confirm Appointment"):
        if not patient_name:
            ...
            conn = sqlite3.connect('health_app.db')
            c = conn.cursor()
            c.execute('''INSERT INTO appointments (patient_name, patient_contact, doctor_name, clinic, date, slot, disease)
                VALUES (?, ?, ?, ?, ?, ?, ?)''',
                (patient_name, patient_contact, selected_doctor, doctor_info['clinic'], str(appointment_date), selected_slot, disease_info))
            conn.commit()
            conn.close()
            st.balloons()
            st.markdown(
    ...
    if st.button("🔍 Find My Appointments"):
        if not search_name:
            ...
            conn = sqlite3.connect('health_app.db')
            df = pd.read_sql_query("SELECT * FROM appointments WHERE patient_name LIKE ?", conn, params=('%' + search_name + '%',))
            conn.close()
            if df.empty:
                st.info("No appointments found for " + search_name)
            else:
                st.success("Found " + str(len(df)) + " appointment(s)!")
    ...
    if st.button("Register Doctor"):
        if not doc_name or not doc_contact:
            ...
            conn = sqlite3.connect('health_app.db')
            c = conn.cursor()
            c.execute("INSERT INTO doctors_reg (name, specialization, clinic, contact, slots, location, lat, lng) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (doc_name, doc_spec, doc_clinic, doc_contact, doc_slots, doc_location, doc_lat, doc_lng))
            conn.commit()
            conn.close()
    ...
    if st.button("Register Hospital"):
        if not hosp_name or not hosp_contact:
            ...
            conn = sqlite3.connect('health_app.db')
            c = conn.cursor()
            c.execute("INSERT INTO hospitals_reg (name, address, contact, speciality, beds, lat, lng) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (hosp_name, hosp_address, hosp_contact, hosp_spec, hosp_beds, hosp_lat, hosp_lng))
            conn.commit()
            conn.close()
    ...
    if st.button("Register Pharmacy"):
        if not pharm_name or not pharm_contact:
            ...
            conn = sqlite3.connect('health_app.db')
            c = conn.cursor()
            c.execute("INSERT INTO pharmacies_reg (name, address, contact, lat, lng) VALUES (?, ?, ?, ?, ?)",
                (pharm_name, pharm_address, pharm_contact, pharm_lat, pharm_lng))
            conn.commit()
            conn.close()
